...
exports.seed = function(knex, Promise) {
  ...
  return Promise.all([
    ...
    .then(function() {
      return Promise.all([
        knex('productions')
          .where({ name: 'Mozart - Magic Flute' })
          .select('id'),
        knex('venues')
          .where({ venue_name: 'Garsington Opera' })
          .select('id'),
      ]);
    })
    ...
    .then(function() {
      return Promise.all([
        knex('productions')
          .where({ name: 'Haydn - The Creation' })
          .select('id'),
        knex('venues')
          .where({ venue_name: 'Maldon Choral Society' })
          .select('id'),
      ]);
    })
    .then(function(values) {
